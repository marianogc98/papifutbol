...
import { uploadToCloudinary, deleteFromCloudinary, isCloudinaryConfigured, isCloudinaryUrl } from './cloudinary'
...
/**
 * Guarda un archivo en Cloudinary o en el sistema de archivos local
 */
export async function saveFile(
  ...
  if (isCloudinaryConfigured()) {
    try {
      const { url } = await uploadToCloudinary(uploadType, file)
      // Extraer nombre del archivo de la URL para mantener compatibilidad
      const urlParts = url.split('/')
      const filename = urlParts[urlParts.length - 1] || generateFileName(file.name)
      
      return {
        filename,
        path: url, // Guardar la URL completa de Cloudinary
      }
    } catch (error: any) {
      // Si falla Cloudinary, intentar con sistema local
      console.warn('Error al subir a Cloudinary, usando sistema local:', error.message)
    }
  }

  // Fallback: Guardar en sistema de archivos local
...
/**
 * Elimina un archivo de Cloudinary o del sistema de archivos local
 */
export async function deleteFile(imagePath: string | null | undefined): Promise<void> {
  if (!imagePath) return

  // Si es una URL de Cloudinary, eliminarla de Cloudinary
  if (isCloudinaryUrl(imagePath)) {
    await deleteFromCloudinary(imagePath)
    return
  }

  // Si es una ruta local (empieza con /uploads/), eliminarla del sistema de archivos
  if (!imagePath.startsWith('/uploads/')) {
    // Es una URL externa que no es Cloudinary, no la eliminamos
    return
  }
